...
// AccentColors returns the active Catppuccin accent palette.
// Order is stable so UI components can pick colors by index.
func AccentColors() []color.Color {
	return []color.Color{
...
// NeutralColors returns the active neutral palette (text + surfaces).
func NeutralColors() []color.Color {
	return []color.Color{
...
// SpectrumColors returns all 26 Catppuccin colors for the active theme.
func SpectrumColors() []color.Color {
	colors := make([]color.Color, 0, 26)
	colors = append(colors, AccentColors()...)
	colors = append(colors, NeutralColors()...)
	return colors
}
...
func ProviderColor(provider string) color.Color {
	...
	// Explicit user overrides from theme.ProviderColors have priority.
	if cfg := config.Get(); cfg != nil {
		if c, ok := cfg.Theme.ProviderColors[provider]; ok && c != "" {
			return lipgloss.Color(c)
		}
		if c, ok := cfg.Theme.ProviderColors[normalized]; ok && c != "" {
			return lipgloss.Color(c)
		}
	}

	if c, ok := defaultProviderUIColor(normalized); ok {
		return c
	}

	// Keep provider-config color for custom providers.
...
func defaultProviderUIColor(provider string) (color.Color, bool) {
	switch provider {
	case "claude":
